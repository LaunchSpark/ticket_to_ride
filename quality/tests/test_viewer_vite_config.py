...
class ViewerViteConfigTests(unittest.TestCase):
    def test_viewer_vite_config_enables_polling(self) -> None:
        vite_config = (REPO_ROOT / "applications" / "viewer" / "vite.config.js").read_text(encoding="utf-8")
        package_json = (REPO_ROOT / "applications" / "viewer" / "package.json").read_text(encoding="utf-8")
