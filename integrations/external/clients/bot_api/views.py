...
def player_view_from_payload(payload: Dict[str, Any]) -> PlayerView:
    ...
    context = PlayerContextView(
        ...
        remaining_time_ms=payload["context"].get("remainingTimeMs"),
        initial_time_ms=payload["context"].get("initialTimeMs"),
        increment_ms=payload["context"].get("incrementMs"),
    )
    return PlayerView(
        player_id=payload["playerId"],
        name=payload["name"],
...
def route_from_payload(payload: Dict[str, Any]) -> Route:
    route_id = payload.get("routeId") or payload.get("routeKey") or (
        f"{payload['city1'].replace(' ', '_')}-{payload['city2'].replace(' ', '_')}-{payload['color']}"
    )
    route = Route(payload["city1"], payload["city2"], payload["length"], payload["color"], route_id)
