...
import { buildStatusState, readParams } from "./services/url-utils.jsx";
...
function App() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const result = await loadMatchPayload(initialParams.apiBase, initialParams.matchId);
        if (isCancelled) {
          return;
        }
  ...
  }, [initialParams.apiBase, initialParams.matchId]);
  ...
  const topBarStatus = deriveTopBarStatus(matchData, currentPointer.roundIndex, currentPointer.turnIndex);
  const canStepBack = previousPointer.changed;
  const canStepForward = nextPointer.changed;
  const statusCard = statusState
    ? h(StatusScreen, {
